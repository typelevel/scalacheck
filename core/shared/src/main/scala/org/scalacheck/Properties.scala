...
/** Represents a collection of properties, with convenient methods for checking all properties at once. <p>Properties
  * are added in the following way:</p>
  *
  * {{{
...
open class Properties(val name: String) {
  ...
  /** Customize the parameters specific to this class.
    *
    * After the command-line (either [[main]] above or sbt) modifies the default parameters, this method is called with
    * the current state of the parameters. This method must then return parameters. The default implementation returns
    * the parameters unchanged. However, a user can override this method in a properties subclass. Their method can
    * modify the parameters. Those parameters will take precedence when the properties are executed.
    */
  ...
  /** Convenience method that checks the properties with the given parameters (or default parameters, if not specified)
    * and reports the result on the console. Should only be used when running tests interactively within the Scala REPL.
    *
  ...
  /** Convenience method that makes it possible to use this property collection as an application that checks itself on
    * execution. Calls `System.exit` with the exit code set to the number of failed properties.
    */
  ...
  /** Adds all properties from another property collection to this one with a prefix this is prepended to each included
    * property's name.
    */
