 ...
 *    1. Various combinators for producing generators of values for more complex data types.
 *
 ...
 *  }}}
 *
 *  The [[Prop]] above is defined with parameters only and without an explicit generator, because generators are
 *  implicitly provided by [[Arbitrary]] for various data types.
 *
 ...
 *  }}}
 *
 *  The above definition isn't necessary, though. The same boolean generator is defined in [[Arbitrary]] as an implicit
 *  declaration for automatically parameterizing [[Prop]]s. Instead, use the generator that is defined in [[Arbitrary]]
 *  and available from the polymorphic method [[Arbitrary.arbitrary]] with an explicit type parameter:
 *
 *  {{{
 ...
 *  }}}
 *
 *  For more information on designing custom generators and the motivations for doing so, see chapter 6, ''Generators in
 *  Detail'', of the book ''ScalaCheck: The Definitive Guide'' (2013) by Rickard Nilsson published by Artima Press.
 *
 ...
 *  }}}
 *
 *  The class methods for [[Gen]] should be familiar with those in the [[scala.collection Scala collections API]]:
 *
 ...
 *
 *  Definitions for generating arbitrary values of commonly used types in Scala are defined elsewhere, see
 *  [[Arbitrary]].
 *
...
sealed abstract class Gen[+T] extends Serializable { self =>
  ...
  /** Evaluate this generator with the given parameters.
   *
   *  The generator will attempt to generate a valid `T` value. If a valid value is not produced it may retry several
   *  times, determined by the `retries` parameter (which defaults to 100).
   *
  ...
  /** Create a new generator that uses this generator to produce a value that fulfills the given condition. If the
   *  condition is not fulfilled, the generator fails (returns None). Also, make sure that the provided test property is
   *  side-effect free, e.g. it should not use external vars.
   */
  def filter(p: T => Boolean): Gen[T] = suchThat(p)

  /** Create a new generator that uses this generator to produce a value that doesn't fulfill the given condition. If
   *  the condition is fulfilled, the generator fails (returns None). Also, make sure that the provided test property is
   *  side-effect free, e.g. it should not use external vars.
   */
  ...
  /** Create a new generator that uses this generator to produce a value that fulfills the given condition. If the
   *  condition is not fulfilled, the generator fails (returns None). Also, make sure that the provided test property is
   *  side-effect free, e.g. it should not use external vars. This method is identical to [Gen.filter].
   */
  ...
  /** Create a generator that calls this generator repeatedly until the given condition is fulfilled. The generated
   *  value is then returned. Make sure that the provided test property is side-effect free (it should not use external
   *  vars).
   *
   ...
   *  vars).
   *
   *  If the generator fails more than 10000 times, a RetryUntilException will be thrown. You can call `retryUntil` with
   *  a second parameter to change this number.
   */
  ...
  /** Returns a new property that holds if and only if both this and the given generator generates the same result, or
   *  both generators generate no result.
   */
...
object Gen extends GenArities with GenVersionSpecific {
  ...
  sealed abstract class Parameters extends Serializable { outer =>
    ...
    /** The size of the generated value. Generator implementations are allowed to freely interpret (or ignore) this
     *  value. During test execution, the value of this parameter is controlled by [[Test.Parameters.minSize]] and
  ...
  object Choose extends time.JavaTimeChoose {
    ...
    /** This method gets a ton of use -- so we want it to be as fast as possible for many of our common cases.
     */
     ...
     *
     *  Note that unlike the choose method, whose bounds are inclusive, this method's upper bound is exclusive. We
     *  determine how many random bits we need (bitLen), and then round up to the nearest number of bytes (byteLen). We
     *  generate the bytes, possibly truncating the most significant byte (bytes(0)) if bitLen is not evenly-divisible
     *  by 8.
     *
     *  Finally, we check to see if the BigInt we ended up with is in our range. If it is not, we restart this method.
     *  The likelihood of needing to restart depends on span. In the worst case we have almost a 50% chance of this
     *  (which occurs when span is a power of 2 + 1) and in the best case we never restart (which occurs when span is a
     *  power of 2).
     */
      ...
      new Choose[Double] {
        def choose(low: Double, high: Double) =
          if (low > high) throw new IllegalBoundsError(low, high)
            ...
            frequency(1 -> const(Double.NegativeInfinity), 9 -> choose(Double.MinValue, high))
          else if (high == Double.PositiveInfinity)
            frequency(1 -> const(Double.PositiveInfinity), 9 -> choose(low, Double.MaxValue))
     ...
     *
     *  The minimum scale used will be 34. That means that the fractional part will have at least 34 digits (more if one
     *  of the given numbers has a scale larger than 34).
     *
    ...
    /** The "scale" of a decimal number refers to the number of digits in the fractional part. For example, 3.0000 has a
     *  scale of 4.
     *
     *  We can generate an arbitrary number of digits in the decimal expansion of a number, so if a user calls choose(0,
     *  1) we need to decide "how much" work to do. The minScale ensures that we do "enough" work to generate
     *  interesting numbers.
     *
     *  The implicit instance fixes this value, but since users may want to use other scales we expose this method as
     *  well.
     */
    ...
    /** Transform a Choose[T] to a Choose[U] where T and U are two isomorphic types whose relationship is described by
     *  the provided transformation functions. (exponential functor map)
     */
   ...
   *
   *  Gen.recursive[List[Int]] { recurse => Gen.choose(0, 10).flatMap { idx => if (idx < 5) recurse.map(idx :: _) else
   *  Gen.const(idx :: Nil) } }
   */
  ...
  /** A generator that generates a random value in the given (inclusive) range. If the range is invalid, an
   *  IllegalBoundsError exception will be thrown.
   */
  ...
  /** Wraps a generator lazily. The given parameter is only evaluated once, and not until the wrapper generator is
   *  evaluated.
   */
  ...
  /** Wraps a generator for later evaluation. The given parameter is evaluated each time the wrapper generator is
   *  evaluated.
   */
  ...
  /** Generates a container of any Traversable type for which there exists an implicit [[org.scalacheck.util.Buildable]]
   *  instance. The elements in the container will be generated by the given generator. The size of the generated
   *  container is limited by `n`. Depending on what kind of container that is generated, the resulting container may
   *  contain fewer elements than `n`, but not more. If the given generator fails generating a value, the complete
   *  container generator will also fail.
   */
  ...
  /** Generates a container of any Traversable type for which there exists an implicit [[org.scalacheck.util.Buildable]]
   *  instance. The elements in the container will be generated by the given generator. The size of the container is
  ...
  def buildableOf[C, T](g: Gen[T])(implicit
    ...
    sized(s => choose(0, Integer.max(s, 0)))
      .flatMap(n => buildableOfN(n, g)(using evb, evt))

  /** Generates a non-empty container of any Traversable type for which there exists an implicit
   *  [[org.scalacheck.util.Buildable]] instance. The elements in the container will be generated by the given
   *  generator. The size of the container is bounded by the size parameter used when generating values.
   */
  ...
  ): Gen[C[T]] = buildableOfN[C[T], T](n, g)(using evb, evt)

  /** A convenience method for calling `buildableOf[C[T],T](g)`. */
  def containerOf[C[_], T](g: Gen[T])(implicit
      evb: Buildable[T, C[T]],
      evt: C[T] => Traversable[T]
  ): Gen[C[T]] = buildableOf[C[T], T](g)(using evb, evt)

  /** A convenience method for calling `nonEmptyBuildableOf[C[T],T](g)`. */
  def nonEmptyContainerOf[C[_], T](g: Gen[T])(implicit
      evb: Buildable[T, C[T]],
      evt: C[T] => Traversable[T]
  ): Gen[C[T]] = nonEmptyBuildableOf[C[T], T](g)(using evb, evt)

  /** Generates a list of random length. The maximum length depends on the size parameter. This method is equal to
   *  calling `containerOf[List,T](g)`.
   */
  def listOf[T](g: => Gen[T]) = buildableOf[List[T], T](g)

  /** Generates a non-empty list of random length. The maximum length depends on the size parameter. This method is
   *  equal to calling `nonEmptyContainerOf[List,T](g)`.
   */
  ...
  /** Generates a map of random length. The maximum length depends on the size parameter. This method is equal to
   *  calling <code>containerOf[Map,(T,U)](g)</code>.
   */
  def mapOf[T, U](g: => Gen[(T, U)]) = buildableOf[Map[T, U], (T, U)](g)

  /** Generates a non-empty map of random length. The maximum length depends on the size parameter. This method is equal
   *  to calling <code>nonEmptyContainerOf[Map,(T,U)](g)</code>.
   */
  def nonEmptyMap[T, U](g: => Gen[(T, U)]) = nonEmptyBuildableOf[Map[T, U], (T, U)](g)

  /** Generates a map with at most the given number of elements. This method is equal to calling
  ...
  /** Takes a function and returns a generator that generates arbitrary results of that function by feeding it with
   *  arbitrarily generated input parameters.
   */
  ...
  /** Generates a character that can represent a valid hexadecimal digit. This includes both upper and lower case
   *  values.
   */
  ...
  /** Generates Double values according to the given gaussian distribution, specified by its mean and standard
   *  deviation.
   *
   ...
   *
   *  This distribution represents the expected number of failures before a successful test, where the probability of a
   *  successful test is p = 1 / (mean + 1).
   *
   *  The ideal range of values is [0, ∞), although the largest value that can be produced here is 2147483647
   *  (Int.MaxValue).
   */
   ...
   *
   *  In principle any positive value is a valid rate parameter. However, our method of generating values cannot handle
   *  large rates, so we require rate <= 745.
   */
  ...
  /** Generates Int values according to the given binomial distribution, specified by the number of trials to conduct,
   *  and the probability of a true test.
   *
  ...
  /** Generates numbers within the given inclusive range, with extra weight on zero, +/- unity, both extremities, and
   *  any special numbers provided. The special numbers must lie within the given range, otherwise they won't be
   *  included.
   */
