...
object Gen extends GenArities{
   ...
   */
  def distinctBuildableOfN[C,T](n: Int, gen: Gen[T], maxCollisions: Int)(areEqual: (T,T) => Boolean)(implicit
                                                                                                     evb: Buildable[T,C], evt: C => Traversable[T]
  ): Gen[C] =
    tailRecM((List.empty[T], 0, 0)) {
    case (_, _, collisions) if collisions >= maxCollisions => Gen.fail
  ...
  def distinctContainerOfN[C[_], T](n: Int, g: Gen[T], m: Int = 1000)(e: (T, T) => Boolean)(implicit
    evb: Buildable[T, C[T]], evt: C[T] => Traversable[T]
