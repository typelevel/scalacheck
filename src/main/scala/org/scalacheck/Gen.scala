...
object Gen extends GenArities{
  ...
  def buildableOfN[C,T](n: Int, g: Gen[T])(implicit
   ...
   // TODO: Can we guarantee c.size == n (See issue #89)? use Sized?
  ...
  def nonEmptyBuildableOf[C,T](g: Gen[T])(implicit
    evb: Buildable[T,C], evt: Sized[C]
  ): Gen[C] =
    sized(s => choose(1,s).flatMap(buildableOfN[C,T](_,g))) suchThat { evt.isNotEmpty }
  ...
  def nonEmptyContainerOf[C[_],T](g: Gen[T])(implicit
    evb: Buildable[T,C[T]], evt: Sized[C[T]]
