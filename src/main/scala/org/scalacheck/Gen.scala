...
object Gen extends GenArities with GenVersionSpecific {
  ...
  /**
   * Generates a container of any Traversable type for which
   * there exists an implicit [[org.scalacheck.util.Buildable]]
   * instance.
   *
   * The elements in the container will be generated by the given
   * generator. The size of the generated container is limited by `n`,
   * and will try to reach the size. Depending on what kind of
   * container that is generated, the resulting container may contain
   * fewer elements than `n`, but not more. If the given generator
   * fails generating a value, the complete container generator will
   * also fail.
   */
