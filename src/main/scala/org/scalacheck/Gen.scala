...
object Gen extends GenArities{
  ...
  case class DistinctBuildableOfNException[T,U](produced: Map[U,T])
    extends RuntimeException(s"distinctBuildableOfN failed after producing ${produced.size} elements (${produced.mkString(",")})")

  /** Generate a container of any Traversable type for which there exists an
   *  implicit [[org.scalacheck.util.Buildable]] instance.
   *
   *  The elements in the container will be generated by the given generator.
   *  If the given generator fails generating a value, the complete container
   *  generator will also fail.
   *
   *  Elements are distinct, which is determined by applying universal equality
   *  to the key derived by `distinctBy`. If `n` distinct elements cannot be
   *  generated, the container generator will fail.
   *
   *  This method has `O(n log n)` growth in `n`, so prefer [[setOfN]] where
   *  applicable, which has roughly `O(n)` growth in `n`.
   *
   *  {{{setOfN[T](n,gen,maxCollisions).map(_.toVector)}}} is equivalent and
   *  preferable to {{{distinctBuildableOfN[Vector[T],T,T](n,gen,maxCollisions)(identity)}}}
   *
   *  @param n the number of distinct elements to generate
   *  @param gen the generator used to produce each element
   *  @param maxCollisions the maxmimum number of collisions before giving up.
   *                       This caps the maxiumum number of trials at `n` + `maxCollisions`.
   */
  def distinctBuildableOfN[C,T,U](n: Int, gen: Gen[T], maxCollisions: Int = 1000)
                                 (distinctBy: T => U)
                                 (implicit evb: Buildable[T,C], evt: C => Traversable[T]): Gen[C] =
    tailRecM((Map.empty[U,T], 0, 0)) {
      case (accum, _, collisions) if collisions >= maxCollisions => throw new DistinctBuildableOfNException(accum)
      case (accum, size, _)       if size >= n                   => Gen.const(Right(accum))
      case (accum, size, collisions) =>
        gen.map { trial: T =>
          val identifier = distinctBy(trial)
          if (accum.contains(identifier)) Left((accum, size, collisions + 1))
          else Left((accum + (identifier -> trial), size + 1, collisions))
        }
    }.map(m => evb.fromIterable(m.values))

  case class DistinctBuildableOfNByFnException[T](produced: List[T])
    extends RuntimeException(s"distinctBuildableOfNByFn failed after producing ${produced.size} elements (${produced.mkString(",")})")
   ...
   *
   *  This method has `O(n^2)` growth in `n`, so prefer either [[setOfN]] or [[distinctBuildableOfN]]
   *  where applicable.
   *
   *  {{{setOfN[T](n,gen,maxCollisions).map(_.toVector)}}} is equivalent and
   *  preferable to {{{distinctBuildableOfNByFn[Vector[T],T](n,gen,maxCollisions)(_ ==_)}}}
   *
   *  {{{distinctBuildableOfN[C,T,U](n,gen,maxCollisions)(_.someKey)}}} is equivalent and
   *  preferable to {{{distinctBuildableOfNByFn[Vector[T],T](n,gen,maxCollisions)(_.someKey === _.someKey)}}}
   *
   *  @param n the number of distinct elements to generate
   *  @param gen the generator used to produce each element
   *  @param maxCollisions the maxmimum number of collisions before giving up.
   *                       This caps the maxiumum number of trials at `n` + `maxCollisions`.
   */
  def distinctBuildableOfNByFn[C,T](n: Int, gen: Gen[T], maxCollisions: Int)
                                   (areEqual: (T,T) => Boolean)
                                   (implicit evb: Buildable[T,C], evt: C => Traversable[T]): Gen[C] =
    tailRecM((List.empty[T], 0, 0)) {
      case (accum, _, collisions) if collisions >= maxCollisions => throw new DistinctBuildableOfNByFnException(accum)
      case (accum, size, _)       if size >= n                   => Gen.const(Right(accum))
      case (accum, size, collisions) =>
        gen.map { trial: T =>
          if (accum.exists(areEqual(_, trial))) Left((accum, size, collisions + 1))
          else Left((trial :: accum, size + 1, collisions))
        }
  ...
  def distinctContainerOfN[C[_],T,U](n: Int, g: Gen[T], m: Int = 1000)
                                    (by: T => U)
                                    (implicit evb: Buildable[T, C[T]], evt: C[T] => Traversable[T]): Gen[C[T]] =
    distinctBuildableOfN(n,g,m)(by)

  /** A convenience method for calling `distinctBuildableOfNByFn[C[T],T](n,g,m)(e)`. */
  def distinctContainerOfNByFn[C[_],T](n: Int, g: Gen[T], m: Int = 1000)
                                      (e: (T, T) => Boolean)
                                      (implicit evb: Buildable[T, C[T]], evt: C[T] => Traversable[T]): Gen[C[T]] =
    distinctBuildableOfNByFn(n, g, m)(e)
  ...
  case class SetOfNException[T](produced: Set[T])
    extends RuntimeException(s"setOfN failed after producing ${produced.size} elements ($produced)")

  /** Generates a set with exactly `n` elements.
   *
   * This is distinct from `buildableOfN[Set[T],T](n,g)`, which will attempt
   * to produce at most `n` elements, and is intentded for the use case
   * when producing less than `n` elements would be invalid input.
   *
   * Distinctness of elements is determined using Scala's universal equality.
   *
   * @param n The number of elements in the generated set. If this is impossible
   *          within `maxCollisions`, the generator will fail
   * @param gen Used to produce the desired values.
   * @param maxCollisions
   */
  def setOfN[T](n: Int, gen: Gen[T], maxCollisions: Int = 1000): Gen[Set[T]] =
    tailRecM((Set.empty[T], 0, 0)) {
      case (accum, _, collisions) if collisions >= maxCollisions => throw new SetOfNException[T](accum)
      case (accum, size, _)       if size >= n                   => Gen.const(Right(accum))
      case (accum, size, collisions) =>
        gen.map { trial: T =>
          if (accum.contains(trial)) Left((accum, size, collisions + 1))
          else Left((accum + trial, size + 1, collisions))
        }
    }
