...
//
// Copy pasted from scala-parser-combinators.jar so that Scalacheck
// itself can be dependency free, i.e. depend only on scala-library.jar
// under the 2.11.x modularization.
//
// This is because scalacheck itself is used to test the compiler, so
// we avoid quite a bit of pain in integration builds by a little duplication here.
//
 ...
 */
private[scalacheck] trait Parsers {
 ...
 * @author Adriaan Moors
 */
private[scalacheck] object NoPosition extends Position {
  def line = 0
  def column = 0
 ...
 */
private[scalacheck] trait Positional {
