...
object codegen {
  ...
  def valsWithTypes(i: Int) =
    idents("t", i) zip
      idents("T", i) map { case (v, t) => s"$v:$t" } mkString ","
  ...
  def arbfn(i: Int) =
    s"""
       ...
       |  implicit def arbFunction${i}[${types(i)},Z](
       |    implicit g: Arbitrary[Z], ${coImplicits(i)}
       |  ): Arbitrary[${fntype(i)}] =
  ...
  def functionCogen(i: Int) = {
    ...
    s"""
       |  implicit final def function${i}[${types(i)}, Z](
       |    implicit ${wrappedArgs("Arbitrary", i)}, z: Cogen[Z]
       |  ): Cogen[$fn] =
